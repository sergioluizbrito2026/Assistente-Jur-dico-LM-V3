...
if page == "Super Admin":
    ...
    with tab_plans:
        with get_connection() as conn:
            plan_rows = conn.execute(
                "SELECT id,name,slug,price_monthly,max_users,max_documents,max_ai_queries,max_storage_mb,features,active,created_at "
                "FROM plans ORDER BY price_monthly,id"
            ).fetchall()
        ...
        st.caption(
            ...
            "A criação/edição comercial de novos planos pode ser adicionada junto ao módulo de cobrança."
        )
        st.markdown('</div>', unsafe_allow_html=True)
    ...
    with tab_usage:
        ...
        usage_rows = []
        for r in sorted(rows, key=lambda x: float(x.get("ai_used") or 0), reverse=True):
            try:
            ...
            try:
                limit = float(r.get("max_ai") or 0)
            except (TypeError, ValueError):
                limit = 0.0

            if limit > 0:
                pct = (ai_used / limit) * 100
                remaining = max(limit - ai_used, 0)
                limit_display = int(limit) if limit.is_integer() else round(limit, 2)
                remaining_display = int(remaining) if remaining.is_integer() else round(remaining, 2)
                usage_display = f"{pct:.0f}%"
            else:
                limit_display = "∞"
                remaining_display = "∞"
            ...
            usage_rows.append({
                ...
                "Plano": r.get("plan", "-"),
                "Utilizado": int(ai_used) if ai_used.is_integer() else round(ai_used, 2),
                "Limite": limit_display,
                "Restante": remaining_display,
                "Uso": usage_display,
                "Status": "ATIVO" if r.get("status") == "active" else "SUSPENSO",
            })
        ...
        if usage_rows:
            ...
            if not chart_df.empty:
                ...
                fig.update_layout(
                    ...
                    yaxis=dict(showgrid=True, gridcolor="rgba(47,88,140,.25)"),
