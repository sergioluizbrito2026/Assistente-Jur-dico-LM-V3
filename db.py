...
def init_db() -> None:
    """Cria as tabelas e índices necessários sem apagar dados existentes."""
...
def seed_demo() -> None:
    ...
    O seed é idempotente para organização, usuário, documentos
    e caso demonstrativos.
    """
    ...
    with get_connection() as c:
        ...
        for name, typ, pages, ocr, demo_chunks in demos:
            ...
            for page, content, chunk_index in demo_chunks:
                c.execute(
                    """
                    INSERT INTO chunks(
                        document_id,
    ...
    # O índice é opcional. Falha de embeddings não deve impedir
...
def database_health() -> Dict[str, Any]:
    ...
    try:
        ...
        return {
            ...
            "connection": bool(row and row["ok"] == 1),
