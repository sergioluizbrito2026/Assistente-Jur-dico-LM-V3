...
CORREÇÃO (item 5 do roadmap): adicionado suporte ao provedor
Claude (Anthropic), seguindo o mesmo padrão já usado para
Gemini e OpenAI. Nenhuma outra camada do sistema precisa mudar —
ai_orchestrator.py, rag_pipeline.py e evaluation.py dependem só
da interface generate_answer(query, chunks, agent_instruction),
que continua igual.

Para usar:
    LLM_PROVIDER=claude
    ANTHROPIC_API_KEY=sk-ant-...
    (opcional) ANTHROPIC_MODEL=claude-sonnet-5 (default)
"""
...
def _get_max_tokens() -> int:
    try:
        value = int(os.getenv("LLM_MAX_TOKENS", "2048"))
...
def _safe_chunks(chunks: Any) -> List[Any]:
    if chunks is None:
        return []
...
def _context(chunks: Sequence[Any] | None) -> str:
    ...
    for index, chunk in enumerate(chunks, 1):
        if isinstance(chunk, dict):
            document = (
                chunk.get("document") or chunk.get("document_name")
                or chunk.get("name") or "Documento desconhecido"
            )
            page = chunk.get("page") if chunk.get("page") is not None else "N/D"
            chunk_id = chunk.get("chunk_id") or chunk.get("id") or "N/D"
            content = (
                chunk.get("content") or chunk.get("text")
                or chunk.get("page_content") or chunk.get("chunk") or ""
            )
        else:
            document = "Documento"
            page = "N/D"
            chunk_id = "N/D"
            content = _safe_text(chunk)

        content = _safe_text(content)
        if not content:
            continue

        parts.append(f"[{index}] DOCUMENTO: {document} | PÁGINA: {page} | CHUNK: {chunk_id}\n{content}")

    if not parts:
        return "Nenhuma evidência documental válida foi recuperada."
...
def get_agent_instruction(agent: str | None) -> str:
    agent = _safe_text(agent).lower()
...
def build_prompt(
    ...
    if agent_instruction:
        parts.extend(["INSTRUÇÃO DO AGENTE:", agent_instruction.strip(), ""])

    parts.extend([
        "CONTEXTO / EVIDÊNCIAS:", context, "",
...
def _get_gemini_client():
    ...
    try:
        ...
        logger.exception("Erro ao inicializar Gemini: %s", exc)
        return None
...
def _get_openai_client():
    ...
    try:
        ...
        return OpenAI(api_key=api_key, timeout=_get_timeout())
    except Exception as exc:
        logger.exception("Erro ao inicializar OpenAI: %s", exc)
        return None


# ============================================================
# CLIENTE CLAUDE (NOVO — item 5)
# ============================================================
...
def _get_anthropic_client():
    ...
    try:
        ...
        return Anthropic(api_key=api_key, timeout=_get_timeout())
    except Exception as exc:
        logger.exception("Erro ao inicializar Claude (Anthropic): %s", exc)
        return None
...
def _generate_openai(prompt: str) -> str:
    ...
    if not response.choices:
        raise RuntimeError("A OpenAI não retornou escolhas.")
...
def _generate_claude(prompt: str) -> str:
    ...
    Segue o mesmo padrão dos outros provedores: o SYSTEM_PROMPT
    vai no campo `system` (equivalente à mensagem "system" da
    OpenAI), e o prompt já montado por build_prompt() (que já
    inclui instrução do agente + contexto + pergunta) vai como
    a mensagem do usuário.
    """

    client = _get_anthropic_client()
    if client is None:
        raise RuntimeError("Cliente Claude (Anthropic) não configurado.")
    ...
    if not content_blocks:
        raise RuntimeError("O Claude não retornou conteúdo.")
    ...
    if not text:
        raise RuntimeError("O Claude não retornou texto na resposta.")
...
def _demo_response(query: str = "", agent: str = AGENT_GENERAL) -> str:
    label = AGENT_LABELS.get(agent, "Agente Geral")
...
`LLM_PROVIDER=gemini` e `GEMINI_API_KEY`

ou:

`LLM_PROVIDER=openai` e `OPENAI_API_KEY`

ou:

`LLM_PROVIDER=claude` e `ANTHROPIC_API_KEY`
...
def _prepare_request(query: Any, chunks: Any = None) -> tuple[str, List[Any]]:
    query_text = _safe_text(query)
    normalized_chunks = _safe_chunks(chunks)
    return (query_text, normalized_chunks)
...
def generate_answer(query, chunks=None, system_prompt=None, agent_instruction=None):
    """
    Interface principal do AI Service. Compatível com:

        generate_answer(query, chunks)
        generate_answer(query, chunks, agent_instruction="...")
        generate_answer(prompt)   # usado pelo orchestrator em fallback
    """

    query_text, chunks_list = _prepare_request(query, chunks)

    if not query_text:
        return "Não foi fornecida uma pergunta."

    provider = _get_provider()

    agent = AGENT_GENERAL
    if agent_instruction:
        instruction_text = _safe_text(agent_instruction).lower()
    ...
    if chunks_list:
        prompt = build_prompt(
            query=query_text, chunks=chunks_list,
            system_prompt=system_prompt, agent_instruction=agent_instruction,
        )
    else:
        if agent_instruction:
            prompt = build_prompt(
                query=query_text, chunks=[],
                system_prompt=system_prompt, agent_instruction=agent_instruction,
            )
        else:
            prompt = query_text
    ...
    if provider == "demo":
        return _demo_response(query=query_text, agent=agent)
    ...
    raise RuntimeError(f"LLM_PROVIDER inválido: {provider}")


def generate_answer_result(query, chunks=None, system_prompt=None, agent_instruction=None) -> Dict[str, Any]:
    provider = _get_provider()
    ...
    try:
        ...
        return {
            "success": bool(answer), "answer": _safe_text(answer),
            "provider": provider, "model": model, "latency_ms": latency_ms,
            "temperature": _get_temperature(), "max_tokens": _get_max_tokens(),
            "error": None,
        }

    except Exception as exc:
        latency_ms = int((time.perf_counter() - started) * 1000)
        logger.exception("Falha na geração da resposta.")

        return {
            "success": False, "answer": "", "provider": provider, "model": model,
            "latency_ms": latency_ms, "temperature": _get_temperature(),
            "max_tokens": _get_max_tokens(),
            "error": {"type": type(exc).__name__, "message": str(exc)[:500]},
        }


def generate_agent_answer(query: str, chunks: Sequence[Any] | None = None, agent: str = AGENT_GENERAL) -> Dict[str, Any]:
    agent = _safe_text(agent).lower()
    if agent not in AGENT_LABELS:
        agent = AGENT_GENERAL

    instruction = get_agent_instruction(agent)
    result = generate_answer_result(query=query, chunks=chunks, agent_instruction=instruction)
    result.update({"agent": agent, "agent_label": AGENT_LABELS[agent]})
    return result


def legal_answer(query: str, chunks: Sequence[Any] | None = None) -> Dict[str, Any]:
    return generate_agent_answer(query=query, chunks=chunks, agent=AGENT_LEGAL)


def risk_answer(query: str, chunks: Sequence[Any] | None = None) -> Dict[str, Any]:
    return generate_agent_answer(query=query, chunks=chunks, agent=AGENT_RISK)


def summary_answer(query: str, chunks: Sequence[Any] | None = None) -> Dict[str, Any]:
    return generate_agent_answer(query=query, chunks=chunks, agent=AGENT_SUMMARY)


def general_answer(query: str, chunks: Sequence[Any] | None = None) -> Dict[str, Any]:
    return generate_agent_answer(query=query, chunks=chunks, agent=AGENT_GENERAL)
...
def ai_status() -> Dict[str, Any]:
    ...
    return {
        ...
        "agents": [
            {"id": AGENT_LEGAL, "name": AGENT_LABELS[AGENT_LEGAL], "status": "active"},
            {"id": AGENT_RISK, "name": AGENT_LABELS[AGENT_RISK], "status": "active"},
            {"id": AGENT_SUMMARY, "name": AGENT_LABELS[AGENT_SUMMARY], "status": "active"},
            {"id": AGENT_GENERAL, "name": AGENT_LABELS[AGENT_GENERAL], "status": "active"},
        ],
    }
...
def clear_ai_cache() -> Dict[str, Any]:
    ...
    return {"success": True, "message": "Cache dos clientes LLM limpo."}


def self_test() -> Dict[str, Any]:
    chunks = [{
        "chunk_id": "chunk-001", "document_id": "doc-001", "document": "Contrato.pdf",
        "page": 1, "content": "O presente contrato tem por objeto a prestação de serviços de consultoria.",
    }]

    try:
        prompt = build_prompt(
            query="Qual é o objeto do contrato?", chunks=chunks,
            agent_instruction=legal_agent_instruction(),
        )
        return {
            "status": "ok", "module": "services.ai", "provider": _get_provider(),
            "model": _get_model(), "prompt_generated": bool(prompt),
            "prompt_length": len(prompt), "agents": list(AGENT_LABELS.keys()),
        }
    except Exception as exc:
        return {
            "status": "error", "module": "services.ai",
            "error": {"type": type(exc).__name__, "message": str(exc)[:500]},
        }
...
if __name__ == "__main__":
    ...
    print("Prompt:", result.get("prompt_generated"))
    print("Agentes:", result.get("agents"))
